<template>
  <div class="activity-sale-info-box limitSale">
    <div class="activity-sale-pro-box">
      <div
        id="product-falsh-sale-timer"
        v-if="serverTime && baseInfo.activityInfo && baseInfo.activityInfo.endTime && baseInfo.activityInfo.countDown > 0"
        class="sales-progress"
      >
        {{ nl('page_year_end_ends_in_capitalize') }}
        <az-count-down :endTime="baseInfo.activityInfo.endTime"></az-count-down>
      </div>
      <div class="activity-sale-info">
        <!-- <az-icon icon-class="icon-icon-test" class="icon-tag" /> -->
        <span v-if="!showCountDownTime" class="activity-sale-title">{{ nl('page_limited_time_title') }}</span>
        <span
          @mouseenter="handleFlashSaleIntro()"
          @mouseleave="handleFlashSaleIntro()"
          v-if="baseInfo.activityInfo && baseInfo.activityInfo.retireDate"
          class="activity-sale-intro"
        >
          ...
          <div :class="[{ hide: !popupSaleTipShow }]" class="introduce-hover-content flash-sale-intro-pop">
            <div id="delay_info" class="ready-to-off-sale">
              <p class="text">{{ delayDesc }} {{ baseInfo.activityInfo && baseInfo.activityInfo.retireDate }}</p>
            </div>
          </div>
        </span>
      </div>
...
export default {
  name: 'LimitSaleTimer',
  languageKeys: ['page_limited_time_title', 'page_year_end_ends_in_capitalize'],
