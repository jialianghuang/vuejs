<template>
  <div :class="{ from_dialog: fromDialog != 'showroom' && fromDialog }" class="custom-size-box">
    <div v-if="!fromDialog" class="custom-label-box">
      <label @click="selectCustom" class="custom-label">
    ...
    <div v-show="isVisible" class="custom_size_form">
      ...
      <div :class="{ isNewDialog: ['cartEdit', 'product'].includes(fromDialog) }" class="size_form">
        <ul class="dress_ul">
          <li v-for="(customItem, index) in customList" :key="customItem.key">
            <div class="label">
              ...
              <a-select
                ...
                :displayTxt="`name`"
                :selectStyle="{ width: '150px', height: '31px' }"
        ...
        <div v-if="!fromDialog || ['cartEdit', 'product'].includes(fromDialog)" class="measure-block">
        ...
        <ul>
          <li>
            <div :class="{ error: confirmCheckboxError }" class="return_support_notice">
              ...
              <span v-html="isBridal ? nl('page_custom_size_agree_des_bridal') : nl('page_custom_size_agree_des')"></span>
            </div>
          </li>
        </ul>
      </div>
...
<script>
import customSizeMixins from '@/assets/js/quickshopCustomSizeMixins.js'
...
export default {
  ...
  methods: {
    ...
    inputBlur(key) {
      ...
      // let noticeDicDes = this.notice_dic_common
      // if (this.baseInfo.catId == 9) {
      //   noticeDicDes = this.notice_dic
      // }
      // if (this.customListForm[key]) {
      //   if (key in noticeDicDes) {
      //     if (this.baseInfo.catId == 9) {
      //       if (this.checkAndNotice(key)) {
      //         if (['bust', 'waist'].includes(key)) {
      //           this.comboCheckInputs()
      //         }
      //       }
      //     } else {
      //       const currentIsRight = this.checkAndNotice(key)
      //       let isUnderBust = false
      //       if (this.customListForm.hasOwnProperty('under_bust')) {
      //         isUnderBust = true
      //       }
      //       let underBustIsRight, waistIsRight, bustIsRight, hipsIsRight, hollowIsRight, heightIsRight
      //       switch (key) {
      //         case 'bust':
      //           underBustIsRight = (isUnderBust && this.checkAndNotice('under_bust')) || false
      //           waistIsRight = (this.customListForm.hasOwnProperty('waist') && this.checkAndNotice('waist')) || false
      //           this.showUnderBust()
      //           if (currentIsRight && underBustIsRight && (this.baseInfo.catId == 45 || isUnderBust)) {
      //             this.bustUnderCheck()
      //           }
      //           if (currentIsRight && waistIsRight) {
      //             this.bustWaistCheck()
      //           }
      //           break
      //         case 'under_bust':
      //           bustIsRight = this.checkAndNotice('bust')
      //           waistIsRight = (this.customListForm.hasOwnProperty('waist') && this.checkAndNotice('waist')) || false
      //           this.showUnderBust()
      //           if (currentIsRight && bustIsRight && (this.baseInfo.catId == 45 || isUnderBust)) {
      //             this.bustUnderCheck()
      //           }
      //           if (waistIsRight && bustIsRight) {
      //             this.bustWaistCheck()
      //           }
      //           break
      //         case 'waist':
      //           hipsIsRight = this.checkAndNotice('hips')
      //           bustIsRight = this.checkAndNotice('bust')
      //           this.showUnderBust()
      //           if (currentIsRight && hipsIsRight) {
      //             this.hipsWaistCheck()
      //           }
      //           if (currentIsRight && bustIsRight) {
      //             this.bustWaistCheck()
      //           }
      //           break
      //         case 'hips':
      ...
      //           break
      //         case 'height':
      //           hollowIsRight = this.checkAndNotice('hollow_to_floor')
      //           if (currentIsRight && hollowIsRight) {
      //             this.heightHollowCheck()
      //           }
      //           break
      //         case 'hollow_to_floor':
      //           heightIsRight = this.checkAndNotice('height')
      //           if (currentIsRight && heightIsRight) {
      //             this.heightHollowCheck()
      //           }
      //           break
      //         default:
            ...
            .unit_item {
              ...
              .faq {
                ...
                em {
                  ...
                  width: 20px;
                  height: 20px;
                  background-image: url(~assets/images/credit_card_pic.png);
