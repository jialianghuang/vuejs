            ...
            <tbody>
              ...
              <tr v-for="(name, i) in tableInfo.rowsName" :key="i">
                <th v-if="!isBoyVest && !sizeChartInfo.isRings" class="th-block">
                  <span :class="['th-text', { 'big-th-block': name.includes('Hollow') }]">
                    {{ nl(name) }}
                    <template v-if="!showShoes || (showShoes && name === 'Foot Length')">
                      <span :class="unit">({{ unit == 'inch' ? 'in' : 'cm' }})</span>
                    </template>
                    <size-chart-hover :unit="true" :name="name.toLowerCase()" :from="'product'"></size-chart-hover>
                  </span>
                </th>
                <th v-else class="th-block">
                  <span :class="{ 'big-th-block': name.includes('Hollow') }" class="th-text">
                    {{ nl(name) }}
                    <size-chart-hover :unit="false" :name="name.toLowerCase()" :from="'product'"></size-chart-hover>
                  </span>
                </th>
                <td
...
export default {
      ...
      switch (this.baseInfo.catId) {
        ...
        default:
          if (this.baseInfo.goodsId == 1002653) {
            ...
            privateInfo.belongCategory = 'other'
          } else {
            privateInfo.belongCategory = 'other'
          }
          break
  ...
  methods: {
    ...
    closeSizeChartLayer() {
      this.$store.commit('product/setSizeChartLayerShow', false)
