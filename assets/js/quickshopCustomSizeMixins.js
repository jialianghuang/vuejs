...
export default {
  ...
  computed: {
    ...mapState({
      ...
      customSizeInfo: (state) => state.quickshop.customSizeInfo
    }),
        ...
        if (this.customListForm[oneKey] && this.customListForm[otherKey]) {
          ...
          if (diffVal < restrictSizeItem[minKey] || diffVal > restrictSizeItem[maxKey]) {
            ...
            if (restrictSizeItem[minKey] >= 0) {
              // diffNotice = `The difference between the ${names[0]} and the ${names[1]} measurements cannot be less than ${restrictSizeItem.inch_min} inches (${restrictSizeItem.cm_min} cm) or greater than ${restrictSizeItem.inch_max} inches (${restrictSizeItem.cm_max} cm)`
              ...
              // diffNotice = `The difference between the ${names[0]} and the ${names[1]} measurements cannot be greater than ${restrictSizeItem.inch_max} inches (${restrictSizeItem.cm_max} cm)`
              diffNotice = `${this.nl('page_customsize_tip_diff')} ${names[0]} ${this.nl('page_customsize_tip_and')} ${names[1]} ${this.nl(
                'page_customsize_tip_greater'
              )} ${restrictSizeItem.inch_max} ${this.nl('page_common_length_inches').toLowerCase()} (${restrictSizeItem.cm_max} cm)`
            } else {
              // diffNotice = `The difference between the ${names[1]} and the ${names[0]} measurements cannot be greater than ${Math.abs(
              //   restrictSizeItem.inch_min * 1
              // )} inches (${Math.abs(restrictSizeItem.cm_min * 1)} cm)`
