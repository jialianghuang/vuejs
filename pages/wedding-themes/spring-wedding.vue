    ...
    <div class="spring-wedding">
      <div class="content">
        ...
        <pick-theme-color
          ...
          :ec="ec"
          :current-color="color"
          :bg-color="springWeddingData.familyBackground"
          :color-family-list="colorFamilyList"
          @select-color="selectColor"
        >
          <gallery-banner
            :theme="theme"
            :image-url="springWeddingData.topBannerUrl"
            :ec="ec"
            :current-color="color"
            :style-gallery-list="springWeddingData.styleGalleryList"
            @getGalleryInfo="getGalleryInfo"
          ></gallery-banner>
        </pick-theme-color>
        <!-- 选择分类 -->
        <choose-category :theme="theme" :category-list="springWeddingData.categoryList"></choose-category>
        <!-- 商品列表 -->
        ...
        <!-- 底部banner -->
        <bottom-banner :image-url="springWeddingData.bottomBannerUrl" :ec="ec"></bottom-banner>
        <!-- seo -->
...
export default {
  ...
  computed: {
    ...
    colorFamilyList() {
      return this.springWeddingData.familyList ? Object.values(this.springWeddingData.familyList) : []
    }
  },
  async asyncData(context) {
    try {
      ...
      const params = {
        theme: 'spring-wedding', // string类型 区分获取的页面信息 spring-wedding / beach-wedding
        'color-family': undefined // 可选 每次点击color-family要重新调用接口更新页面内容
      }

      const [{ data: springWeddingData }, colorHexRes] = await Promise.all([
  ...
  methods: {
    selectColor(item) {
      ...
      if (index > -1) {
        this.color = item.key
        this.springWeddingData = this.lazyloadData[index]
      } else {
    ...
    async getProductListData() {
      try {
        ...
        const asyncList = this.springWeddingData.categoryList.map((item) => {
        ...
        if (res && res2 && res3 && res4 && res5) {
          ...
          // 给每个组件绑定module id
          this.moduleList = this.springWeddingData.categoryList
            ? this.springWeddingData.categoryList.map((item) => {
          ...
          resList.forEach((item) => {
            ...
            const index = this.springWeddingData.categoryList.findIndex((i) => {
              return i.value === item.name
            })
            // 将对应res赋值到moduleList
            if (item.name === this.springWeddingData.categoryList[index].value) {
    ...
    async getLazyloadData() {
      try {
        const asyncList = this.springWeddingData.familyList.map((item) => {
    ...
    quickShopClick(params) {
      ...
      this.setOrderSourceFlag('wedding_theme_spring_wedding', true)
    },
    addSuccess() {
      this.getCartInfo()
